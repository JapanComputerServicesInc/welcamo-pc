...
@section('title', __('menu.approvals'))
...
<div class="container-fluid">
    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            <form class="form-row" method="POST" action="{{ route('search_approvals') }}">
                @csrf
            ...
            <table class="table">
                <thead class="thead-light">
                    <tr>
                        ...
                        <th scope="col">{{ __('app.approval') }}</th>
                    </tr>
                </thead>
                </body>
                    ...
                    <tr>
                        ...
                        <td>{{ $row->last_dt->format('Y年 n月 j日') }}</td>
                        <td>{{ $row->last_dt->format('G時 i分') }}</td>
                        ...
                        <td>
                            <form action="{{ route('history') }}" method="POST">
                                ...
                                <input type="hidden" name="bname"  value="{{ __('menu.approvals') }}">
                                <input type="hidden" name="broute" value="search_approvals">
                            </form>
                        </td>
                        <td>
                            <form action="{{ route('approval') }}" method="POST">
                                @csrf
                                <button type="submit" class="btn btn-primary btn-block" name="approval">
                                    {{ __('app.approval') }}
                                </button>
                                <input type="hidden" name="id" value="{{ $row->id }}">
                            </form>
